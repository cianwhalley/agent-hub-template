...
Unit defaults: `AGENT_MODEL=cursor-grok-4.6-medium` and `AGENT_MODEL_FALLBACK=latest,sonnet,sol` (newer same-options Grok → Sonnet 5 → GPT Sol). Override either per job with `model:` / `fallback_model:` (`off` disables fallback). Jobs run separately; retired / rate-limited / degraded models walk the chain. Final failure is recorded, posted directly to Slack, and exits non-zero.
