...
`--worker-dir` is repeatable. The **first** path is the picker label (dashboard identity). Extra directories register `repo=` routing; Cursor does **not** add extra My Machines rows. From a secondary repo window, pick the same worker `--name`. See `.cursor/skills/my-machines/SKILL.md`.
